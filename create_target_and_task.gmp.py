...
def get_autogenerated_targets(gmp: Gmp):
  """
  ...
  for target in gmp.get_targets(filter_string="name~zit_ rows=-1").xpath('target'):
    targets['id'] = target.xpath('./@id')
    targets['name'] = target.xpath('name/text()')
    targets['hosts'] = target.xpath('hosts/text()')
    targets['in_use'] = target.xpath('in_use/text()')
