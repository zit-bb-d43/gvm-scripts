...
def get_autogenerated_targets(gmp: Gmp) -> pd.DataFrame:
  ...
  """
  targets = pd.DataFrame(columns=['id','name','hosts','in_use'])
...
def create_target(gmp,url):
  ...
  """
  return gmp.create_target(name=prefix_target+url, hosts=[url], port_list_id=portlist_id)

def create_task(gmp, url, target_id):
  """
  creates a task for given target with predefined scanner, config, alert
  """
  return gmp.create_task(name=prefix_task+url, config_id=config_id, target_id=target_id, scanner_id=scanner_id, alterable=True, alert_ids=[alert_id], preferences=preferences)
